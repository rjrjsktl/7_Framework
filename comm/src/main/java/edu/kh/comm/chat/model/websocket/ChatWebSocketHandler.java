...
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    ----------------------------------------------------------------------------
        ...
        지금 이 class는 설계도임 그래서 bean등록 해줘야함!!!!
	 */
	...
	// 클라이언트 와 연결이 완료되고, 통신할 준비가 되면 수행 (==> new SockJS() )
	@Override
	...
	// 클라이언트로 부터 텍스트 메시지를 전달 받았을 때 수행
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {	
		...
		if(result > 0) {
			...
			// -> Set<WebSocketSesions> 에서 같은 방 클라이언트만 골라내기
			
			for(WebSocketSession s : sessions) {
				
				// WebSocketSession == HttpSession(로그인정보, 채팅방번호)을 가로챈것
